...
// Gerar codigo unico para certificado
...
export async function GET(request: NextRequest) {
    try {
        ...
        return await withTenant(workspaceId, async (client) => {
            // Buscar progresso geral do usuario
            let query: string;
            let params: any[];

            if (moduloId) {
                // Progresso de um modulo especifico
                query = `
                    SELECT
                        p.lesson_id,
                        p.completed_at,
                ...
                // Progresso geral (ultima licao, total, etc.)
                query = `
                    SELECT
                        p.lesson_id,
                        p.completed_at,
            ...
            `;
            const statsResult = await client.query(statsQuery, [user.id, workspaceId]);
            const stats = statsResult.rows[0];

            // Ultima licao acessada (para "Continue de onde parou")
            const lastLessonQuery = `
                ...
                LIMIT 1
            `;
            const lastLessonResult = await client.query(lastLessonQuery, [user.id]);
            const lastLesson = lastLessonResult.rows[0] || null;

            // Proxima licao a fazer (se tiver ultima licao)
            let nextLesson = null;
            if (lastLesson) {
                const nextQuery = `
                    SELECT
                    ...
                    LIMIT 1
                `;
                const nextResult = await client.query(nextQuery, [user.id]);
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!lesson_id) {
            return NextResponse.json(
                { error: 'lesson_id e obrigatorio' },
                { status: 400 }
            );
        }

        return await withTenant(workspaceId, async (client) => {
            // Verificar se a licao existe
            const lessonQuery = `
            ...
            `;
            const lessonResult = await client.query(lessonQuery, [lesson_id]);
            ...
            const progressQuery = `
                ...
                RETURNING *
            `;
            const progressResult = await client.query(progressQuery, [workspaceId, user.id, lesson_id]);

            // Verificar se completou todas as licoes do modulo
            ...
            if (moduleCompleted) {
                ...
                if (existingCert.rows.length === 0) {
                    ...
                    const certQuery = `
                        ...
                        RETURNING *
                    `;
                    const certResult = await client.query(certQuery, [workspaceId, user.id, lesson.modulo_id, certCode]);
                    certificate = {
                        ...certResult.rows[0],
            ...
            return NextResponse.json({
                ...
                message: moduleCompleted
                    ? 'Parabens! Voce completou o modulo!'
                    : 'Licao marcada como completa'
            });
        });
    } catch (error) {
