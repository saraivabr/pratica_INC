...
export async function GET(request: NextRequest) {
    try {
        ...
        const codigo = searchParams.get('codigo'); // Para verificar um certificado especifico
        ...
        return await withTenant(workspaceId, async (client) => {
            // Se buscando certificado especifico por codigo
            if (codigo) {
                const certQuery = `
                    SELECT
                    ...
                    WHERE c.codigo = $1
                `;
                const certResult = await client.query(certQuery, [codigo]);
            ...
            // Listar todos os certificados do usuario
            const query = `
                SELECT
                    c.id, c.codigo, c.emitido_em,
                    m.id as modulo_id, m.nome as modulo_nome, m.slug as modulo_slug,
            ...
            // Modulos disponiveis para certificacao (em progresso ou nao iniciados)
            ...
            `;
            const availableResult = await client.query(availableQuery, [user.id, workspaceId]);
