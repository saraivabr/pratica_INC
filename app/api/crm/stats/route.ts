...
export async function GET(request: NextRequest) {
  try {
    ...
    return await withTenant(workspaceId, async (client) => {
      ...
      const leadsThisMonthRes = await client.query(`
      ...
      const leadsLastMonthRes = await client.query(`
        SELECT COUNT(*) as count
        FROM cvcrm_leads
        WHERE workspace_id = $1
      ...
      const conversionLastMonthRes = await client.query(`
        SELECT
          COUNT(*) FILTER (WHERE
            LOWER(situacao_nome) LIKE '%ganho%'
            OR LOWER(situacao_nome) LIKE '%fechado%'
      ...
      const revenueLastMonthRes = await client.query(`
        SELECT COALESCE(SUM(valor_negocio), 0) as revenue
        FROM cvcrm_leads
        WHERE workspace_id = $1
        AND data_cad >= $2 AND data_cad < $3
      ...
      const originStatsRes = await client.query(`
        SELECT
      ...
      const leadsPerDayRes = await client.query(`
        SELECT
      ...
      const hotLeadsNoContactRes = await client.query(`
        SELECT COUNT(*) as count
        FROM cvcrm_leads
        WHERE workspace_id = $1
        AND score >= 70
