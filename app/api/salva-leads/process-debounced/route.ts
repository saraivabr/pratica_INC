...
async function processConversation(
  ...
  // Contexto para Redis
  const context = { workspaceId, phone: conv.lead_phone };

  // Tentar adquirir lock (evita processamento duplicado)
  const { acquired, lockValue } = await acquireLock(workspaceId, conv.lead_phone);
  if (!acquired) {
  ...
  try {
    ...
    try {
      response = await processWithAgent(conv, combinedMessage, workspaceId);
    ...
    await releaseLock(workspaceId, conv.lead_phone, lockValue);
  }
}
