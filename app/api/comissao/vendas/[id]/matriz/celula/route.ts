...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const { rows: parcelaRows } = await dbQuery(
    ...
    const { rows: celulaRows } = await dbQuery(
...
export async function POST(request: NextRequest, { params }: Params) {
  try {
    ...
    const { rows: celulasCorretor } = await dbQuery(
    ...
    try {
      ...
      const { rows: celulasAtualizadas } = await dbQuery(
