...
export async function GET(request: NextRequest) {
  try {
    ...
    // 1. Total pago no periodo - filter by workspace_id
    const totalPeriodoResult = await dbQuery(
      `SELECT
        COUNT(*) as quantidade,
        COALESCE(SUM(valor), 0) as total
       FROM pagamentos_intermediacao
       WHERE workspace_id = $1 AND data_pagamento >= $2 AND data_pagamento <= $3`,
      [ctx.workspaceId, inicio, fim]
    );
    ...
    // 2. Pagamentos por metodo - filter by workspace_id
    ...
    // 3. Pagamentos por beneficiario (top 10) - filter by workspace_id
    const porBeneficiarioResult = await dbQuery(
      `SELECT
       ...
       WHERE pg.workspace_id = $1 AND pg.data_pagamento >= $2 AND pg.data_pagamento <= $3
    ...
    // 4. Evolucao mensal - filter by workspace_id
    ...
    // 5. Estatisticas de parcelas - filter by workspace_id
    ...
    // 6. Proximos vencimentos (7 dias) - filter by workspace_id
    ...
    // 7. Media de dias para pagamento (pontualidade) - filter by workspace_id
