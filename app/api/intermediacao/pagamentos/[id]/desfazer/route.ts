...
export async function POST(
  ...
  try {
    ...
    // Buscar pagamento - filter by workspace_id
    ...
    const outrasParcelasPagasResult = await dbQuery(
      `SELECT COUNT(*) as count
       ...
       WHERE p.venda_id = $1 AND p.id != $2 AND p.status = 'paga' AND p.workspace_id = $3`,
      [pagamento.venda_id, pagamento.parcela_id, ctx.workspaceId]
    );

    const outrasParcelasPagas = parseInt(outrasParcelasPagasResult.rows[0]?.count || "0");

    // Excluir pagamento - filter by workspace_id
    await dbQuery(
      `DELETE FROM pagamentos_intermediacao WHERE id = $1 AND workspace_id = $2`,
      [pagamentoId, ctx.workspaceId]
    );

    // Reverter status da parcela - filter by workspace_id
    await dbQuery(
      ...
      [pagamento.parcela_id, ctx.workspaceId]
    );

    // Se a venda estava 'paga' e agora tem parcela pendente, reverter status
    const vendaResult = await dbQuery(
      `SELECT status FROM vendas_intermediacao WHERE id = $1 AND workspace_id = $2`,
      [pagamento.venda_id, ctx.workspaceId]
    );
    ...
    // Registrar auditoria CRITICA - include workspace_id
    await dbQuery(
      `INSERT INTO log_auditoria_intermediacao
       (workspace_id, entidade, entidade_id, acao, usuario_id, usuario_nome, dados_anteriores, dados_novos, justificativa, criticidade, created_at)
    ...
    // Log adicional para a parcela - include workspace_id
    await dbQuery(
      `INSERT INTO log_auditoria_intermediacao
       (workspace_id, entidade, entidade_id, acao, usuario_id, usuario_nome, dados_novos, justificativa, criticidade, created_at)
