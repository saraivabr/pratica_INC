...
export async function GET(
  ...
  try {
    ...
    const result = await dbQuery(query, [parcelaId, ctx.workspaceId]);
...
export async function PUT(
  ...
  try {
    ...
    // Verificar se a parcela existe e seu status (filtered by workspace_id)
    ...
    if (valor !== undefined && valor !== parseFloat(parcela.valor)) {
      // Buscar todas as outras parcelas do mesmo beneficiario/venda (filtered by workspace_id)
      const outrasParcelasResult = await dbQuery(
        `SELECT SUM(valor) as soma
         FROM parcelas_intermediacao
         WHERE venda_id = $1 AND beneficiario_id = $2 AND id != $3 AND workspace_id = $4`,
        [parcela.venda_id, parcela.beneficiario_id, parcelaId, ctx.workspaceId]
      );
    ...
    // Update parcela (with workspace_id filter for safety)
    const updateQuery = `
      UPDATE parcelas_intermediacao
      SET ${updateFields.join(", ")}
      WHERE id = $${updateIndex} AND workspace_id = $${updateIndex + 1}
      RETURNING *
    `;
    updateParams.push(parcelaId, ctx.workspaceId);
    ...
    await dbQuery(
      `INSERT INTO log_auditoria_intermediacao
       (entidade, entidade_id, acao, usuario_id, usuario_nome, dados_anteriores, dados_novos, workspace_id, created_at)
