...
export async function POST(
  ...
  try {
    ...
    const parcelaResult = await dbQuery(
      `SELECT * FROM parcelas_intermediacao WHERE id = $1 AND workspace_id = $2`,
    ...
    // Atualizar status da parcela para 'cancelada' (with workspace_id filter for safety)
    await dbQuery(
    ...
    await dbQuery(
      `INSERT INTO log_auditoria_intermediacao
       (entidade, entidade_id, acao, usuario_id, usuario_nome, dados_anteriores, dados_novos, justificativa, criticidade, workspace_id, created_at)
