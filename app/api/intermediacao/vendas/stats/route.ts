...
export async function GET(request: NextRequest) {
  try {
    ...
    // Construir condicao de periodo - workspace_id sempre primeiro
    let periodoCondition = "";
    const periodoParams: any[] = [ctx.workspaceId];
    let paramIndex = 2;
    ...
    const { rows: totaisRows } = await dbQuery(
      ...
      FROM im_vendas
      WHERE workspace_id = $1 ${periodoCondition}`,
      periodoParams
    );
    ...
    const { rows: porPeriodo } = await dbQuery(
      `SELECT
        ...
        COALESCE(SUM(valor_comissao), 0) as comissao_total
      FROM im_vendas
      WHERE workspace_id = $1 AND data_venda IS NOT NULL ${periodoCondition}
    ...
    const { rows: topBeneficiarios } = await dbQuery(
      ...
      WHERE v.workspace_id = $1 ${periodoCondition.replace(/data_venda/g, 'v.data_venda')}
    ...
    if (dataInicio && dataFim) {
      ...
      const { rows: periodoAnterior } = await dbQuery(
        `SELECT
          ...
          COALESCE(SUM(valor_comissao), 0) as total_comissao
        FROM im_vendas
        WHERE workspace_id = $1 AND data_venda BETWEEN $2 AND $3`,
        [ctx.workspaceId, inicioAnterior.toISOString().split('T')[0], fimAnterior.toISOString().split('T')[0]]
      );
