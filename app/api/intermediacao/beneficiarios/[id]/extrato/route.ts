...
export async function GET(
  ...
  try {
    ...
    const beneficiarioResult = await dbQuery(
      `SELECT id, codigo, nome, email, telefone FROM intermediacao_beneficiarios WHERE id = $1 AND workspace_id = $2`,
    ...
    // Construir filtros
    let whereClause = "WHERE c.beneficiario_id = $1 AND c.workspace_id = $2";
    const queryParams: any[] = [beneficiarioId, ctx.workspaceId];
    let paramIndex = 3;
