...
export async function GET(
  ...
  try {
    ...
    const beneficiarioResult = await dbQuery(
      `SELECT id, codigo, nome FROM intermediacao_beneficiarios WHERE id = $1 AND workspace_id = $2`,
    ...
    let periodoFilter = " AND c.workspace_id = $2";
    const periodoParams: any[] = [beneficiarioId, ctx.workspaceId];
    let paramIndex = 3;
    ...
    const proximasQuery = `
      ...
      ORDER BY p.data_vencimento ASC
    `;
    const proximasResult = await dbQuery(proximasQuery, [beneficiarioId, ctx.workspaceId]);
