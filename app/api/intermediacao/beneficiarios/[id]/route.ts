...
async function registrarAuditoria(
  ...
  await dbQuery(
    `INSERT INTO intermediacao_auditoria
     (workspace_id, entidade, entidade_id, operacao, usuario_id, dados_anteriores, dados_novos, justificativa, created_at)
...
export async function GET(
  ...
  try {
    ...
    `;
    const beneficiarioResult = await dbQuery(beneficiarioQuery, [beneficiarioId, ctx.workspaceId]);
    ...
    const comissoesQuery = `
      ...
      WHERE c.beneficiario_id = $1 AND c.workspace_id = $2
      ORDER BY c.created_at DESC
    `;
    const comissoesResult = await dbQuery(comissoesQuery, [beneficiarioId, ctx.workspaceId]);
    ...
    `;
    const parcelasResult = await dbQuery(parcelasQuery, [beneficiarioId, ctx.workspaceId]);
    ...
    const saldosQuery = `
      ...
      WHERE c.beneficiario_id = $1 AND c.workspace_id = $2
    `;
    const saldosResult = await dbQuery(saldosQuery, [beneficiarioId, ctx.workspaceId]);
...
export async function PUT(
  ...
  try {
    ...
    const beneficiarioAtual = await dbQuery(
      `SELECT * FROM intermediacao_beneficiarios WHERE id = $1 AND workspace_id = $2`,
      [beneficiarioId, ctx.workspaceId]
    );
    ...
    const comissoesPagasQuery = `
      ...
      FROM intermediacao_parcelas p
      JOIN intermediacao_comissoes c ON p.comissao_id = c.id
      WHERE c.beneficiario_id = $1 AND c.workspace_id = $2 AND p.status = 'pago'
    `;
    const comissoesPagasResult = await dbQuery(comissoesPagasQuery, [beneficiarioId, ctx.workspaceId]);
    ...
    if (documento && tipo_documento) {
      ...
      const existeDoc = await dbQuery<{ id: number }>(
        `SELECT id FROM intermediacao_beneficiarios WHERE documento = $1 AND id != $2 AND workspace_id = $3`,
        [documentoLimpo, beneficiarioId, ctx.workspaceId]
      );
...
export async function DELETE(
  ...
  try {
    ...
    const comissoesPendentesQuery = `
      ...
      FROM intermediacao_parcelas p
      JOIN intermediacao_comissoes c ON p.comissao_id = c.id
      WHERE c.beneficiario_id = $1 AND c.workspace_id = $2 AND p.status = 'pendente'
    `;
    const comissoesPendentesResult = await dbQuery(comissoesPendentesQuery, [beneficiarioId, ctx.workspaceId]);
