...
export async function GET(request: NextRequest) {
  try {
    ...
    `;
    const totaisResult = await dbQuery(totaisQuery, [ctx.workspaceId]);
    ...
    const porCargoQuery = `
      ...
      ORDER BY total DESC
    `;
    const porCargoResult = await dbQuery(porCargoQuery, [ctx.workspaceId]);
    ...
    const rankingQuery = `
      ...
      LEFT JOIN intermediacao_comissoes c ON c.beneficiario_id = b.id AND c.workspace_id = $1
      LEFT JOIN intermediacao_parcelas p ON p.comissao_id = c.id
      WHERE b.ativo = true AND b.workspace_id = $1
      GROUP BY b.id, b.codigo, b.nome, b.cargo
      HAVING COALESCE(SUM(c.valor), 0) > 0
      ORDER BY total_comissoes DESC
      LIMIT 10
    `;
    const rankingResult = await dbQuery(rankingQuery, [ctx.workspaceId]);
    ...
    `;
    const comissoesStatsResult = await dbQuery(comissoesStatsQuery, [ctx.workspaceId]);
    ...
    const novosPorMesQuery = `
      SELECT
        ...
        COUNT(*) as total
      FROM intermediacao_beneficiarios
      WHERE workspace_id = $1 AND created_at >= CURRENT_DATE - INTERVAL '6 months'
      GROUP BY DATE_TRUNC('month', created_at)
      ORDER BY mes DESC
    `;
    const novosPorMesResult = await dbQuery(novosPorMesQuery, [ctx.workspaceId]);

    // Beneficiários sem comissões recentes (últimos 90 dias)
    const semComissoesRecentesQuery = `
      SELECT COUNT(*) as total
      FROM intermediacao_beneficiarios b
      WHERE b.workspace_id = $1 AND b.ativo = true
      AND NOT EXISTS (
    ...
    `;
    const semComissoesRecentesResult = await dbQuery(semComissoesRecentesQuery, [ctx.workspaceId]);
    ...
    `;
    const porTipoDocumentoResult = await dbQuery(porTipoDocumentoQuery, [ctx.workspaceId]);
