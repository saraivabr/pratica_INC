...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.query<QualificacaoData>(
      `SELECT
        q.id,
        q.plantao_id,
        q.total_ofertas,
    ...
    if (result.rows.length === 0) {
      ...
      const plantaoResult = await pool.query<{ meta_ofertas: number }>(
    ...
    const totalQualificadosResult = await pool.query<{ total: number }>(
