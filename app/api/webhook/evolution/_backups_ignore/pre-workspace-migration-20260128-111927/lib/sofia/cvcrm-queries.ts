...
export async function getLeadsByCorretor(
  ...
  // Build WHERE clauses
  const conditions: string[] = ["tenant_id = $1", "corretor_id = $2"];
  const params: any[] = [tenantId, parseInt(corretorId, 10)];
  let paramIndex = 3;
  ...
  const { rows: breakdownRows } = await dbQuery(
    ...
    ORDER BY count DESC`,
    [tenantId, parseInt(corretorId, 10)]
  );
...
export async function getComissoesCorretor(corretorId: string, tenantId: number): Promise<{
  comissoes: Comissao[];
  resumo: {
  ...
  const { rows: reservaComissoes } = await dbQuery(
    ...
    WHERE rc.tenant_id = $1 AND rc.corretor_id = $2`,
    [tenantId, corretorIdNum]
  );
...
export async function getProximasAtividades(
  ...
  const { rows: agendamentos } = await dbQuery(
    ...
    AND (
      ...
      OR lead_id IN (SELECT cvcrm_id FROM cvcrm_leads WHERE tenant_id = $1 AND corretor_id = $3)
    )
    AND data_inicio >= $4
    AND data_inicio <= $5
    AND (status IS NULL OR LOWER(status) NOT IN ('cancelado', 'cancelada', 'concluido', 'concluida'))
    ORDER BY data_inicio ASC`,
    [tenantId, `%${corretorIdNum}%`, corretorIdNum, hoje.toISOString(), dataLimite.toISOString()]
  );
  ...
  const { rows: tarefas } = await dbQuery(
    ...
    INNER JOIN cvcrm_leads l ON l.cvcrm_id = t.cvcrm_lead_id AND l.tenant_id = $1
    WHERE t.tenant_id = $1
    ...
    [tenantId, corretorIdNum, hoje.toISOString(), dataLimite.toISOString()]
  );
...
export async function getRankingEquipe(
  ...
  const params: any[] = [tenantId, start.toISOString(), end.toISOString()];
  let paramIndex = 4;
  ...
  const { rows } = await dbQuery(
    ...
    LEFT JOIN cvcrm_corretores c ON c.cvcrm_id = r.corretor_id AND c.tenant_id = $1
    WHERE ${whereClause}
  ...
  const { rows: leadsRows } = await dbQuery(
    ...
    FROM cvcrm_leads
    WHERE tenant_id = $1 AND data_cadastro_cvcrm >= $2 AND data_cadastro_cvcrm <= $3
...
export async function getMetasCorretor(corretorId: string, tenantId: number): Promise<{
  metas: MetaCorretor[];
  resumo: {
    total_metas: number;
  ...
  const { rows: vendasRows } = await dbQuery(
    ...
    [tenantId, corretorIdNum, startOfMonth.toISOString(), endOfMonth.toISOString()]
  );
