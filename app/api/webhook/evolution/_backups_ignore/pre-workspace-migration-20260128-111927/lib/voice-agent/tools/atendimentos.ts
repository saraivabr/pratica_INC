    ...
    if (cliente_id) {
      ...
      const leadsResult = await dbQuery(`
        SELECT
          ...
          corretor
        FROM cvcrm_leads
        WHERE tenant_id = $1 AND idlead = $2
      ...
      const reservasResult = await dbQuery(`
        SELECT
          ...
          corretor_nome
        FROM cvcrm_reservas
        WHERE tenant_id = $1 AND cliente_principal_id = $2
      ...
      const leadsResult = await dbQuery(`
        SELECT
          ...
          corretor
        FROM cvcrm_leads
        WHERE tenant_id = $1 AND telefone LIKE $2
        ORDER BY data_cad DESC
        LIMIT 20
      ...
      if (result.leads.length > 0) {
        ...
        if (clienteIds.length > 0) {
          ...
          const reservasResult = await dbQuery(`
            ...
            FROM cvcrm_reservas
            WHERE tenant_id = $1 AND cliente_principal_id = ANY($2)
            ORDER BY data_reserva DESC
            LIMIT 20
          `, [tenantId, clienteIds])
          result.reservas = reservasResult.rows
          ...
          const atendimentosResult = await dbQuery(`
            SELECT
              ...
              unidade_nome
            FROM cvcrm_atendimentos
            WHERE tenant_id = $1 AND cliente_id = ANY($2)
            ORDER BY data_abertura DESC
            LIMIT 20
          `, [tenantId, clienteIds])
          result.atendimentos = atendimentosResult.rows
...
const get_agenda_hoje: VoiceAgentToolDefinition = {
  ...
  execute: async (args: Record<string, any>, tenantId: number) => {
    ...
    const tarefasQuery = `
      ...
      LIMIT 50
    `
    const tarefasParams = corretor_id ? [tenantId, ...baseParams, corretor_id] : [tenantId, ...baseParams]
    ...
    const visitasQuery = `
      ...
      LIMIT 50
    `
    const visitasParams = corretor_id ? [tenantId, ...baseParams, corretor_id] : [tenantId, ...baseParams]
    ...
    const agendamentosQuery = `
      ...
      LIMIT 50
    `
    const agendamentosResult = await dbQuery(agendamentosQuery, [tenantId, ...baseParams])
  ...
  execute: async (args: Record<string, any>, tenantId: number) => {
    ...
    let query = `
      ...
      WHERE tenant_id = $1
        ...
        AND status != 'concluida'
    `
    const params: any[] = [tenantId, today.toISOString()]
...
const get_empreendimento_stats: VoiceAgentToolDefinition = {
  ...
  execute: async (args: Record<string, any>, tenantId: number) => {
    ...
    let empreendimentosQuery = `
      ...
      WHERE tenant_id = $1
    `
    const empreendimentosParams: any[] = [tenantId]
    ...
    const stats = await Promise.all(
      empreendimentosResult.rows.map(async (emp: any) => {
        ...
        const leadsResult = await dbQuery(`
          ...
          FROM cvcrm_leads
          WHERE tenant_id = $1 AND empreendimento::text LIKE $2
        `, [tenantId, `%"id":${emp.cvcrm_id}%`])
        ...
        const reservasResult = await dbQuery(`
          ...
          FROM cvcrm_reservas
          WHERE tenant_id = $1 AND empreendimento_id = $2
        `, [tenantId, emp.cvcrm_id])
