...
export async function syncAssistenciasCore(tenantId: number, fullSync = false): Promise<void> {
  ...
  try {
    ...
    const logResult = await pool.query(
      ...
      ['assistencias-core', 'cvcrm_assistencias', tenantId, 'running']
    );
    ...
    const upsertQuery = `
      INSERT INTO cvcrm_assistencias (
        tenant_id, idassistencia, situacao, idsituacao,
      ...
      ON CONFLICT (tenant_id, idassistencia) DO UPDATE SET
