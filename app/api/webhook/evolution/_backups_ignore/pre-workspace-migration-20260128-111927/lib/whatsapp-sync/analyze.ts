 ...
 * @param tenantId - ID do tenant para analise
 * @returns Resultado consolidado da analise
 */
export async function analyzeChats(tenantId: number): Promise<SyncAnalysisResult> {
  console.log(`[WhatsApp Analyze] Iniciando analise para tenant ${tenantId}`);
  ...
  for (const chat of chats) {
    try {
      ...
      const lead = await matchChatToLead(tenantId, chat.phone_number);
...
export async function matchChatToLead(
  ...
  const result = await pool.query<{
    ...
    `SELECT
     ...
     LIMIT 1`,
    [tenantId, lastNineDigits, lastEightDigits]
  );
 ...
 */
export async function getAnalysisStats(tenantId: number): Promise<{
  totalChats: number;
  ...
  const result = await pool.query(
    `SELECT
     ...
     WHERE tenant_id = $1 AND is_group = FALSE`,
    [tenantId]
  );
