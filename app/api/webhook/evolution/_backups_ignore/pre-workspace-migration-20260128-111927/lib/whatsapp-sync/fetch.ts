...
export async function fetchAllChats(
  ...
  console.log(`[WhatsApp Sync] Fetching chats for tenant ${tenantId}, instance ${instanceName}`);
...
export async function fetchAllContacts(
  ...
  console.log(`[WhatsApp Sync] Fetching contacts for tenant ${tenantId}, instance ${instanceName}`);
...
export async function syncChatsToDatabase(
  ...
  console.log(`[WhatsApp Sync] Starting chat sync for tenant ${tenantId}`);

  try {
    const chats = await fetchAllChats(tenantId, instanceName);
...
export async function syncContactsToDatabase(
  ...
  console.log(`[WhatsApp Sync] Starting contact sync for tenant ${tenantId}`);

  try {
    const contacts = await fetchAllContacts(tenantId, instanceName);
...
export async function syncMessagesToDatabase(
  ...
  console.log(`[WhatsApp Sync] Starting message sync for tenant ${tenantId}, ${messagesPerChat} messages per chat`);
  ...
  try {
    ...
    for (const chat of individualChats) {
      try {
        ...
        const messages = await fetchChatMessages(tenantId, instanceName, chat.remoteJid, messagesPerChat);
