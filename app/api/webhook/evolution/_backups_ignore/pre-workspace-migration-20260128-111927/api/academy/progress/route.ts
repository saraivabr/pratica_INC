    ...
    try {
        ...
        const statsQuery = `
            SELECT
                ...
                (SELECT COUNT(*) FROM academy_lessons WHERE ativo = true AND tenant_id = $2) as total_licoes,
                (SELECT COUNT(*) FROM academy_certificates WHERE user_id = $1) as total_certificados
        `;
        const statsResult = await pool.query(statsQuery, [user.id, tenantId]);
...
export async function POST(request: NextRequest) {
    try {
        ...
        const progressQuery = `
            INSERT INTO academy_progress (tenant_id, user_id, lesson_id, completed_at)
            VALUES ($1, $2, $3, NOW())
            ON CONFLICT (user_id, lesson_id) DO UPDATE SET completed_at = NOW()
            RETURNING *
        `;
        const progressResult = await pool.query(progressQuery, [tenantId, user.id, lesson_id]);
        ...
        if (moduleCompleted) {
            ...
            if (existingCert.rows.length === 0) {
                ...
                const certQuery = `
                    INSERT INTO academy_certificates (tenant_id, user_id, modulo_id, codigo, emitido_em)
                    VALUES ($1, $2, $3, $4, NOW())
                    RETURNING *
                `;
                const certResult = await pool.query(certQuery, [tenantId, user.id, lesson.modulo_id, certCode]);
                certificate = {
                    ...certResult.rows[0],
