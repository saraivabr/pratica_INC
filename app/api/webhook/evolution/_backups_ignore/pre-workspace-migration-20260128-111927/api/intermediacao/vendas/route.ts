 ...
 */
async function gerarCodigoVenda(tenantId: number): Promise<string> {
  const now = new Date();
  const yearMonth = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}`;

  const { rows } = await dbQuery(
    `SELECT codigo FROM im_vendas
     WHERE codigo LIKE $1 AND tenant_id = $2
     ORDER BY codigo DESC
     LIMIT 1`,
    [`VND-${yearMonth}-%`, tenantId]
  );
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Gerar codigo unico
    const codigo = await gerarCodigoVenda(ctx.tenantId);
    ...
    try {
      ...
      const { rows: vendaRows } = await dbQuery(
        `INSERT INTO im_vendas (
          tenant_id, codigo, valor_total, valor_comissao, unidade, empreendimento,
