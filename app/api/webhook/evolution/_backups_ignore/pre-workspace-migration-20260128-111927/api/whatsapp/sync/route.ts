 ...
 *
 * GET /api/whatsapp/sync?tenantId=X
 * Retorna última sincronização e estatísticas
 */

import { NextRequest, NextResponse } from 'next/server';
import { getTenant, tenantQuery } from '@/lib/tenant-context';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Obter tenantId do body ou do usuário autenticado
    ...
    if (!tenant) {
      return NextResponse.json(
        { success: false, error: `Tenant ID ${tenantId} não encontrado.` },
        { status: 404 }
      );
    }
    ...
    const createRunResult = await pool.query<SyncRunResult>(
      `INSERT INTO whatsapp_sync_runs (tenant_id, status, started_at, chats_synced, contacts_synced, leads_matched, opportunities_found)
    ...
    console.log(`[Sync] Iniciando sincronização run_id=${runId} tenant_id=${tenantId} instance=${instanceName}`);

    // 2. Sincronizar chats
    let chatsSynced = 0;
    try {
      const chatsResult = await syncChatsToDatabase(tenantId, instanceName);
    ...
    try {
      const contactsResult = await syncContactsToDatabase(tenantId, instanceName);
    ...
    try {
      const messagesResult = await syncMessagesToDatabase(tenantId, instanceName, 50);
    ...
    try {
      const analysisResult = await analyzeChats(tenantId);
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Obter tenantId da query ou do usuário
    const searchParams = request.nextUrl.searchParams;
    let tenantId: number;

    const queryTenantId = searchParams.get('tenantId');
    if (queryTenantId) {
      tenantId = Number(queryTenantId);
    } else {
      const tenant = await findUserTenant(user);
      if (!tenant) {
        return NextResponse.json(
    ...
    const chatsResult = await pool.query(
      `SELECT COUNT(*) as total FROM whatsapp_chats WHERE tenant_id = $1`,
      [tenantId]
    );
    ...
    const contactsResult = await pool.query(
      `SELECT COUNT(*) as total FROM whatsapp_contacts WHERE tenant_id = $1`,
      [tenantId]
    );
    ...
    const messagesResult = await pool.query(
      `SELECT COUNT(*) as total FROM whatsapp_messages WHERE tenant_id = $1`,
      [tenantId]
    );
