...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tenant) {
      const imobiliariaId = (user as any).imobiliaria_id || (user as any).imobiliarias?.id;
      return NextResponse.json({
        ...
        pairingCode: null,
        error: imobiliariaId
          ? `Empresa (ID: ${imobiliariaId}) não configurada. Entre em contato com o suporte.`
          : "Você ainda não tem uma empresa configurada. Inicie a conexão para criar.",
      });
    }

    // Buscar instância do USUÁRIO primeiro (não do tenant)
    const userId = (user as any).id;
    const userInstanceName = (user as any).evolution_instance_name;

    // Se o usuário tem uma instância própria, usar ela
    // Senão, verificar no tenant (fallback para compatibilidade)
    const instances = tenant.evolution_instances || [];
    const tenantInstance = instances.find((i: any) =>
      userInstanceName ? i.instance_name === userInstanceName : true
    ) || instances[0];

    const instanceName = userInstanceName || tenantInstance?.instance_name;
