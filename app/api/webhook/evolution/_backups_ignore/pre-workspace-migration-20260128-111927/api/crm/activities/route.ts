...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await dbQuery(
      `INSERT INTO activities (tenant_id, lead_id, user_id, title, description, activity_type, scheduled_at, priority)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
      [tenantId, lead_id, user_id, title, description, activity_type, scheduled_at, priority || 'medium']
    );
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const res = await dbQuery(
      `UPDATE activities SET ${updates.join(", ")} WHERE tenant_id = $1 AND id = $${paramIndex} RETURNING *`,
      params
    );
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const res = await dbQuery(
      `DELETE FROM activities WHERE tenant_id = $1 AND id = $2 RETURNING id`,
      [tenantId, id]
    );
