...
export async function GET(
  ...
  try {
    ...
    const eventoCheck = await pool.query(
      'SELECT id FROM eventos WHERE id = $1 AND tenant_id = $2',
      [eventoId, tenantId]
    );
    ...
    // Montar query
    let whereClause = 'WHERE evento_id = $1 AND tenant_id = $2';
    ...
    `;
    const statsResult = await pool.query(statsQuery, [eventoId, tenantId]);
...
export async function POST(
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO evento_convidados (evento_id, tenant_id, nome, celular, origem, cvcrm_id, status)
      VALUES ${placeholders}
      RETURNING *
    `;
  ...
  try {
    ...
    const result = await pool.query(
      `DELETE FROM evento_convidados
       WHERE evento_id = $1 AND tenant_id = $2 AND id = ANY($3)
       RETURNING id`,
      [eventoId, tenantId, convidado_ids]
    );
