  ...
  try {
    ...
    if (!tenantIdParam) {
      return NextResponse.json(
        { success: false, error: 'tenantId e obrigatorio' },
        { status: 400 }
      );
    }
    ...
    if (isNaN(tenantId)) {
      return NextResponse.json(
        { success: false, error: 'tenantId deve ser um numero' },
        { status: 400 }
      );
    }
    ...
    const dataQuery = `
      ...
      FROM whatsapp_synced_chats wsc
      LEFT JOIN cvcrm_leads l ON l.tenant_id = wsc.tenant_id AND l.idlead::text = wsc.matched_lead_id
      WHERE ${whereClause}
      ORDER BY
