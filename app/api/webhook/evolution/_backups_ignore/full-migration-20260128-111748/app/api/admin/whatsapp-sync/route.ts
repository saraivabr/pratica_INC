 ...
 * Body (opcional):
 * {
 *   "tenantId": 1,           // Filtrar por tenant específico
...
export async function POST(request: NextRequest) {
  ...
  const filterTenantId = body.tenantId;
    ...
    for (const instance of instances) {
      ...
      if (!tenantId) {
        results.push({
          ...
          tenantId: 0,
          status: 'skipped',
          error: 'tenant_id não encontrado',
        });
        continue;
      }
      ...
      console.log(`[WhatsApp Sync] Processando ${instanceName} (tenant: ${tenantId}, conectado: ${isConnected})`);
      ...
      try {
        ...
        const messagesResult = await syncMessagesToDatabase(tenantId, instanceName, messagesLimit);
        ...
        `, [tenantId, chatsResult.synced, contactsResult.synced]);
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    body: {
      tenantId: 'number (opcional) - Filtrar por tenant específico',
