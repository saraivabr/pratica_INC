 ...
 * Cada ferramenta tem acesso ao contexto da conversa (SalvaLeadsConversation) e ao tenantId.
 */
...
async function executeGetImoveis(
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[Salva-Leads] get_imoveis:', args, 'tenantId:', tenantId);
  }
...
async function executePostVisita(
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('[Salva-Leads] post_visita:', args, 'tenantId:', tenantId);
  }
  ...
  try {
    ...
    await dbQuery(
      `INSERT INTO salva_leads_visitas
       (tenant_id, lead_phone, lead_name, cod_imovel, data_sugerida, periodo, observacao, created_at)
...
export async function executeTool(
  ...
  return tool.execute(args, context, tenantId);
}
