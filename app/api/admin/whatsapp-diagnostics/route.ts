...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const instanceToUser = new Map<string, { id: string; nome: string; workspace_id: number }>();
    const userIdToInfo = new Map<string, { nome: string; workspace_id: number }>();
    ...
    for (const instance of instances) {
      ...
      try {
        ...
        if (userInfo) {
          workspaceId = userInfo.workspace_id;
        ...
        const expectedWebhookUrl = workspaceId
