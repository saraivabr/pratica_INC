...
import { BookTemplate } from '@/components/pdf-templates';
  ...
  try {
    ...
    const pdfBuffer = await renderToBuffer(
      createElement(BookTemplate, {
        empreendimento: {
          ...
          imagemPrincipal: empreendimento.imagem_principal || empreendimento.foto,
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="book-${empreendimento.nome.replace(/\s+/g, '-')}.pdf"`,
      },
    });
  } catch (error) {
    console.error('Erro ao gerar PDF do book:', error);
    return NextResponse.json(
