...
export async function GET(request: NextRequest) {
  try {
    ...
    const conversationsRes = await dbQuery(`
      SELECT
    ...
    const topIntentsRes = await dbQuery(`
      SELECT
    ...
    const resolutionRes = await dbQuery(`
      SELECT
        COUNT(*) as total,
    ...
    const avgResponseTimeRes = await dbQuery(`
    ...
    const unknownIntentsRes = await dbQuery(`
    ...
    const satisfactionRes = await dbQuery(`
    ...
    const categoryDistributionRes = await dbQuery(`
      SELECT
        context->>'last_category' as category,
        COUNT(*) as count
      FROM conversations c
    ...
    const topFlowsRes = await dbQuery(`
      SELECT
    ...
    const frustrationRes = await dbQuery(`
      SELECT
    ...
    const conversationsPerDayRes = await dbQuery(`
      SELECT
    ...
    // 11. Horarios de pico
    const peakHoursRes = await dbQuery(`
      SELECT
