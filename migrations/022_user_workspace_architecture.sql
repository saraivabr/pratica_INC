...
-- BEGIN; -- Removido para permitir execução parcial
...
BEGIN
  FOR user_record IN 
    SELECT id, name AS nome, evolution_instance_name, evolution_connected
    FROM users
    WHERE NOT EXISTS (
...
-- COMMIT; -- Removido para permitir execução parcial
