 ...
 */
export async function getResumoGeral(workspaceId: number, corretor_id?: string) {
  ...
  return withTenant(workspaceId, async (client) => {
    ...
    const { rows: agendamentosRows } = await client.query(
      `
      SELECT
    ...
    const { rows: simulacoesRows } = await client.query(
      `
      SELECT
 ...
 */
export async function getLeadsParaFollowup(workspaceId: number, corretor_id?: string) {
  const corretorFilter = corretor_id ? 'AND l.user_id = $1' : '';
  const params = corretor_id ? [corretor_id] : [];
 ...
 */
export async function getPerformanceCorretor(workspaceId: number, corretor_id: string, periodo: PeriodoAnalytics = '30d') {
  const dateFilter = getPeriodoFilter(periodo);

  const resumo = await getResumoGeral(workspaceId, corretor_id);

  // Tendências (comparar com período anterior)
  const periodoAnterior = periodo === '7d' ? '14 days' : periodo === '30d' ? '60 days' : '180 days';

  return withTenant(workspaceId, async (client) => {
    const { rows: tendenciaRows } = await client.query(
      `
      SELECT
