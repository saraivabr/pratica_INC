...
export async function syncLeadsInteracoes(workspaceId: number, fullSync = false): Promise<void> {
  console.log(`\n🔄 Starting Leads Interações Sync (Tenant ${workspaceId})...`);
  ...
  try {
    ...
    const logResult = await pool.query(
      ...
      ['leads-interacoes', 'cvcrm_leads_interacoes', workspaceId, 'running']
    );
...
if (require.main === module) {
  ...
  syncLeadsInteracoes(workspaceId, true)
    .then(() => process.exit(0))
    .catch((error) => {
      console.error(error);
      process.exit(1);
