...
async function runAllAgents(workspaceId: number, parallel = false): Promise<void> {
  ...
  if (parallel) {
    ...
    const promises = agents.map(async (agent) => {
      ...
      try {
        await agent.fn(workspaceId, true);
...
const workspaceId = args.find(arg => !arg.startsWith('-')) ? parseInt(args.find(arg => !arg.startsWith('-'))!) : 1;
const parallel = args.includes('--parallel') || args.includes('-p');

// Run
if (require.main === module) {
  console.log(`Running sync for workspace_id: ${workspaceId}`);
  console.log(`Usage: npx tsx lib/sync/agents-simple/run-all.ts [workspace_id] [--parallel]\n`);

  runAllAgents(workspaceId, parallel)
    .then(() => {
