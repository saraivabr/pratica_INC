...
export class LeadsTarefasAgent extends BaseSyncAgent<CVCRMTarefaData, DBTarefaData> {
  constructor() {
    const config: AgentConfig = {
      ...
      endpoints: [
        {
          path: '/api/v1/comercial/leads/tarefas',
        ...
        {
          path: '/api/v1/comercial/leads/visitas',
          tokenEnvVar: 'CVCRM_TOKEN_LEAD',
          method: 'GET',
          supportsIncremental: true,
          paginationType: 'offset',
        ...
        {
          path: '/api/v1/comercial/leads/workflow_tempo',
          tokenEnvVar: 'CVCRM_TOKEN_LEAD',
          method: 'GET',
          supportsIncremental: true,
          paginationType: 'offset',
  ...
  async sync(fullSync = false) {
    console.log('[LeadsTarefasAgent] SKIPPED - endpoints not available in CV CRM API');
    const empty = { total: 0, created: 0, updated: 0, skipped: 0, errors: 0, results: [], duration: 0, startedAt: new Date(), completedAt: new Date() };
    return { tarefas: empty, visitas: empty, workflow_tempo: empty };
  }
}
