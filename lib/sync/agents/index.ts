...
export async function syncLeadsDomain(fullSync = false) {
  ...
  // Use allSettled so one agent failure doesn't block the rest
  ...
  const results = settled.map((r, i) => {
    ...
    const names = ['leads-core', 'leads-conversoes', 'leads-interacoes', 'leads-tarefas', 'leads-historico'];
    console.error(`[${names[i]}] Sync failed:`, r.reason?.message || r.reason);
