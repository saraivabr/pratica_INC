...
export async function addMessage(
  ...
  role: string,
  content: string,
  context?: { workspaceId: number; phone: string }
): Promise<void> {
...
export async function appendPendingMessage(
  ...
  if (context) {
    const redisResult = await addToDebounce(context.workspaceId, context.phone, text);
    return redisResult;
  }
...
export async function clearPendingMessages(
  ...
  if (context) {
    await clearDebounce(context.workspaceId, context.phone);
  }
...
export async function getPendingMessages(
  ...
  if (context) {
    const redisMessages = await getDebounceMessages(context.workspaceId, context.phone);
...
export async function getConversationHistoryForAI(
  ...
  const redisHistory = await getHistory(workspaceId, phone, 10);
