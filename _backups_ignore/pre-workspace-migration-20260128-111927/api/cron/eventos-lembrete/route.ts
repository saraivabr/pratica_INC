...
import { getTenant, Tenant } from '@/lib/tenant-context';
...
// Evento ja tem tenant_id no tipo base, usamos o tipo diretamente
...
async function getConvidadosParaLembrete(
  ...
  const { rows } = await pool.query<ConvidadoParaLembrete>(`
    SELECT id, evento_id, tenant_id, nome, celular, status, lembrete_enviado_at
 ...
 */
async function getTenantsAtivosComEvolution(): Promise<Array<{ id: number; name: string }>> {
...
async function processarEventoLembretes(
  ...
  const convidados = await getConvidadosParaLembrete(evento.id, evento.tenant_id);
...
async function processarTenantLembretes(
  ...
  try {
    ...
    if (!instanceName) {
      console.log(`[Eventos-Lembrete] Tenant ${tenantId}: sem instancia Evolution configurada`);
      return result;
    }
    ...
    if (!connected) {
      console.log(`[Eventos-Lembrete] Tenant ${tenantId}: instancia ${instanceName} nao conectada`);
      return result;
    }
    ...
    if (eventos.rows.length === 0) {
      console.log(`[Eventos-Lembrete] Tenant ${tenantId}: nenhum evento para lembrete`);
      return result;
    }

    console.log(`[Eventos-Lembrete] Tenant ${tenantId} "${tenantName}": ${eventos.rows.length} evento(s) para processar`);
    ...
    result.erros++;
    console.error(`[Eventos-Lembrete] Erro processando tenant ${tenantId}:`, error);
    return result;
  }
}
...
export async function GET(request: NextRequest): Promise<NextResponse<CronResponse>> {
  ...
  try {
    ...
    const tenants = await getTenantsAtivosComEvolution();
