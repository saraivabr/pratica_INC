...
import { updateTenant, findUserTenant, createTenant } from "@/lib/tenant-context";
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Body vazio ou inválido, continua normalmente
    }
    ...
    if (freshConnection && instanceName) {
      ...
      await updateTenant(tenantId, { evolution_instances: [] });
    }
      ...
      try {
        ...
        await createInstance({
          ...
          webhook: {
            ...
            // Autenticação do webhook para validação no endpoint receptor
    ...
    return NextResponse.json({
      ...
      instanceName,
      channelId: `poll-${tenantId}-${Date.now()}`,
    });
