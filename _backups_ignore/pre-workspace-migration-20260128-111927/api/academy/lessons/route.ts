...
export async function GET(request: NextRequest) {
    try {
        ...
        if (licaoSlug && moduloSlug) {
            ...
            `;
            const lessonResult = await pool.query(lessonQuery, [tenantId, moduloSlug, licaoSlug]);
        ...
        let whereClause = 'WHERE l.tenant_id = $1 AND l.ativo = true';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const query = `
            INSERT INTO academy_lessons (tenant_id, modulo_id, slug, titulo, conteudo, resumo, duracao_minutos, ordem)
