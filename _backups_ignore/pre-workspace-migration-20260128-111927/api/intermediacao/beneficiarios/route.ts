...
async function registrarAuditoria(
  ...
  await dbQuery(
    `INSERT INTO intermediacao_auditoria
     (tenant_id, entidade, entidade_id, operacao, usuario_id, dados_anteriores, dados_novos, created_at)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existeDoc = await dbQuery<{ id: number }>(
      `SELECT id FROM intermediacao_beneficiarios WHERE documento = $1 AND tenant_id = $2`,
      [documentoLimpo, ctx.tenantId]
    );
    ...
    const codigo = await gerarCodigoBeneficiario(ctx.tenantId);

    // Inserir beneficiário
    const insertQuery = `
      INSERT INTO intermediacao_beneficiarios (
        tenant_id, codigo, nome, tipo_documento, documento, cargo, email, telefone,
