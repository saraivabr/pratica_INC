...
export async function POST(
  ...
  try {
    ...
    // Verificar se ja existe pagamento (filtered by tenant_id)
    ...
    const pagamentoResult = await dbQuery(
      `INSERT INTO pagamentos_intermediacao
       (parcela_id, beneficiario_id, valor, data_pagamento, metodo, comprovante, referencia, registrado_por, tenant_id, created_at)
    ...
    // Atualizar status da parcela para 'paga' (with tenant_id filter for safety)
    await dbQuery(
    ...
    // Verificar se todas as parcelas da venda foram pagas (filtered by tenant_id)
    const parcelasPendentesResult = await dbQuery(
      `SELECT COUNT(*) as pendentes
       FROM parcelas_intermediacao
       WHERE venda_id = $1 AND status != 'paga' AND tenant_id = $2`,
      [parcela.venda_id, ctx.tenantId]
    );

    const parcelasPendentes = parseInt(parcelasPendentesResult.rows[0]?.pendentes || "0");

    // Se todas pagas, atualizar status da venda (with tenant_id filter for safety)
    ...
    await dbQuery(
      `INSERT INTO log_auditoria_intermediacao
       (entidade, entidade_id, acao, usuario_id, usuario_nome, dados_novos, tenant_id, created_at)
