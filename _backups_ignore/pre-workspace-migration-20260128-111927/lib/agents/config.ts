 ...
 */
export async function getAgentConfigs(tenantId: number): Promise<AgentConfig[]> {
  try {
    const { rows } = await dbQuery(
  ...
  try {
    const { rowCount } = await dbQuery(
      `UPDATE agent_configs
       ...
       WHERE tenant_id = $1 AND instance_name = $2`,
      [tenantId, instanceName, isActive]
    );
  ...
  try {
    await dbQuery(
      `INSERT INTO agent_conversation_logs (
        tenant_id, agent_config_id, instance_name, phone_number, lead_id,
...
export async function getConversationLogs(
  ...
  try {
    ...
    const conditions = ['tenant_id = $1', 'instance_name = $2'];
    const params: (number | string)[] = [tenantId, instanceName];
    let paramIndex = 3;
