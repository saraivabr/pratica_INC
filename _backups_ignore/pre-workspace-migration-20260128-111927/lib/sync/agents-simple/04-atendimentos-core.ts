...
export async function syncAtendimentosCore(tenantId: number, fullSync = false): Promise<void> {
  ...
  try {
    ...
    const logResult = await pool.query(
      ...
      ['atendimentos-core', 'cvcrm_atendimentos', tenantId, 'running']
    );
    ...
    const upsertQuery = `
      INSERT INTO cvcrm_atendimentos (
        tenant_id, idatendimento, nome, email, telefone, documento,
      ...
      ON CONFLICT (tenant_id, idatendimento) DO UPDATE SET
        nome = EXCLUDED.nome,
    ...
    for (const atendimento of atendimentos) {
      try {
        ...
        if (atendimento.arquivos && atendimento.arquivos.length > 0) {
          ...
          await pool.query(
            'DELETE FROM cvcrm_atendimentos_arquivos WHERE tenant_id = $1 AND idatendimento = $2',
            [tenantId, atendimento.idatendimento]
          );
          ...
          for (const arquivo of atendimento.arquivos) {
            await pool.query(
              `INSERT INTO cvcrm_atendimentos_arquivos (
                tenant_id, idatendimento, idarquivo, nome, servidor, tipo, tamanho, data_cad, url
