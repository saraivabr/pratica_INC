...
export async function syncLeadsTarefas(tenantId: number, fullSync = false): Promise<void> {
  ...
  try {
    ...
    const logResult = await pool.query(
      ...
      ['leads-tarefas', 'cvcrm_leads_tarefas', tenantId, 'running']
    );
    ...
    const upsertQuery = `
      ...
      ON CONFLICT (tenant_id, idtarefa) DO UPDATE SET
