...
export async function runLeadRecoveryForCorretor(
  ...
  if (process.env.NODE_ENV === 'development') {
    log(`[Salva-Leads] Iniciando recuperacao para corretor ${corretorId} (tenant: ${tenantId})`);
  }

  // 1. Buscar leads candidatos usando funcao existente do lead-recovery
  const candidates = await fetchCandidates(tenantId, corretorId, maxLeads, corretorName);
  if (process.env.NODE_ENV === 'development') {
  ...
  for (const lead of candidates) {
    ...
    try {
      ...
      const alreadyProcessed = await wasRecentlyProcessed(tenantId, lead.atendimentoId);
      if (alreadyProcessed) {
        results.push({
      ...
      if (instanceName) {
        ...
        if (!isValidWhatsApp) {
          ...
          await markPhoneAsInvalid(tenantId, lead.atendimentoId, lead.leadPhone);
          results.push({
            lead: lead.atendimentoId,
            status: 'skipped',
      ...
      if (instanceName) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[Salva-Leads] Tenant ${tenantId} sem instance Evolution configurada`);
        }
        results.push({
          lead: lead.atendimentoId,
          status: 'error',
  ...
  await logRun(tenantId, corretorId, candidates.length, sent, results);
 ...
 */
async function getCachedLeadsFromCVCRM(tenantId: number): Promise<any[]> {
  const cacheKey = `tenant:${tenantId}:leads`;

  return leadsCache.getOrSet(cacheKey, async () => {
    console.log(`[Salva-Leads] Buscando leads do CV CRM para tenant ${tenantId}...`);
...
async function fetchCandidates(
  ...
  try {
    ...
    const allLeads = await getCachedLeadsFromCVCRM(tenantId);
  ...
  try {
    await dbQuery(
      `INSERT INTO salva_leads_conversations
         (tenant_id, atendimento_id, lead_phone, status, classification, context)
...
async function logRun(
  ...
  await dbQuery(
    `INSERT INTO salva_leads_runs
       (tenant_id, corretor_id, scheduled_for, status, leads_processed, leads_sent, results)
     VALUES ($1, $2, NOW(), 'completed', $3, $4, $5)`,
    [tenantId, corretorId, processed, sent, JSON.stringify(results)]
  );
}
...
export async function runLeadRecoveryForTenant(
  ...
  if (process.env.NODE_ENV === 'development') {
    log(`[Salva-Leads] Iniciando execucao para tenant ${tenantId}`);
  }

  const corretores = await getCorretoresComWhatsAppAtivo(tenantId);
  if (process.env.NODE_ENV === 'development') {
  ...
  for (const corretor of corretores) {
    const result = await runLeadRecoveryForCorretor(tenantId, corretor.id, {
      dryRun,
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(
      `[Salva-Leads] Tenant ${tenantId} concluido: ${totalProcessed} processados, ${totalSent} enviados`
    );
  }
