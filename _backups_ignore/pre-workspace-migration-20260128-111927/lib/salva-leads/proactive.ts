...
export async function scheduleProactiveJob(
  ...
  await dbQuery(
    `INSERT INTO salva_leads_proactive_jobs
     (id, trigger_id, lead_phone, corretor_id, tenant_id, scheduled_for, status, attempts)
     VALUES ($1, $2, $3, $4, $5, $6, 'pending', 0)`,
    [id, job.triggerId, job.leadPhone, job.corretorId, job.tenantId, job.scheduledFor]
  ).catch(err => {
 ...
 */
export async function getPendingProactiveJobs(tenantId?: number): Promise<ProactiveJob[]> {
  const query = tenantId
    ? `SELECT * FROM salva_leads_proactive_jobs WHERE status = 'pending' AND scheduled_for <= NOW() AND tenant_id = $1`
