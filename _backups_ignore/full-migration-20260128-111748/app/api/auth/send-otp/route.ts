...
export async function POST(request: Request) {
  try {
    ...
    const { rows: userRows } = await dbQuery(
      `select * from users where telefone = $1 limit 1`,
      [normalizedPhone]
    );
    ...
    if (isBypassUser) {
      ...
      const { rows: sessionRows } = await dbQuery(
        `insert into sessions (user_id, otp_code, otp_expires_at, is_verified, expires_at)
         ...
         returning id`,
        [user.id, otpCode, expiresAt.toISOString(), sessionExpires.toISOString()]
      );
    ...
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000); // 5 min for magic link/code
    const sessionExpires = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days for session
