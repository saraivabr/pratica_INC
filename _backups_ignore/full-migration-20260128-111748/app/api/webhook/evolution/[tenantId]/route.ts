...
import { getTenant, updateTenant, tenantQuery } from '@/lib/tenant-context';
import {
...
export async function POST(
  ...
  try {
    ...
    console.log(`[Webhook Evolution] Tenant ${tenantId}:`, {
      event: body.event,
 ...
 * IMPORTANTE: Filtra por tenant_id para garantir isolamento entre tenants
 */
async function findUserByPhone(phone: string, tenantId: number) {
  ...
  for (const format of uniqueFormats) {
    const { rows } = await dbQuery(
      `select u.*, i.nome as imobiliaria_nome
       ...
       where u.telefone = $1 and u.tenant_id = $2
       limit 1`,
      [format, tenantId]
    );
    const data = rows[0];
  ...
  const { rows } = await dbQuery(
    `select u.*, i.nome as imobiliaria_nome
     from users u
     left join imobiliarias i on i.id = u.imobiliaria_id
     where u.telefone like $1 and u.tenant_id = $2
     limit 1`,
    [`%${lastDigits}`, tenantId]
  );
  const data = rows[0];
 ...
 */
async function handleQRCodeUpdate(tenantId: number, data: any) {
  ...
  if (instanceIndex !== -1) {
    ...
    await updateTenant(tenantId, { evolution_instances: instances as any[] });
 ...
 */
async function handleConnectionUpdate(tenantId: number, data: any) {
  const tenant = await getTenant(tenantId);
  if (!tenant) return;
 ...
 */
async function handleNewMessage(tenantId: number, data: any) {
  try {
    ...
    if (messageId) {
      await dbQuery(
        `INSERT INTO whatsapp_messages (
          tenant_id, instance_name, phone_number, message_id, message_type,
    ...
    if (messageText) {
      try {
        ...
        const agentConfig = await getAgentConfig(tenantId, data.instance);
        ...
        if (agentConfig && shouldEscalate(agentConfig, messageText)) {
          ...
          // Log escalação
          await logConversation(tenantId, {
        ...
        // ============================================
        const eventoContext = await buscarConvidadoPorTelefone(tenantId, phoneNumber);
        ...
        if (eventoContext && deveUsarFluxoEventos(eventoContext)) {
          ...
          if (resultado.resposta) {
            ...
            const user = await findUserByPhone(phoneNumber, tenantId);
...
async function handleSalvaLeadsCheck(
  ...
  const salvaLeadsConv = await getConversationByPhone(tenantId, phoneNumber);
  ...
  await handleSalvaLeadsResponse(tenantId, salvaLeadsConv, messageText, instanceName);
  return true;
}
 ...
 */
async function handleMessageUpdate(tenantId: number, data: any) {
  try {
