...
export async function POST(request: NextRequest) {
  try {
    ...
    // Buscar parcelas - filter by tenant_id
    ...
    const parcelasResult = await dbQuery(
      `SELECT p.*, v.id as venda_id
       ...
       WHERE p.id IN (${placeholders}) AND p.tenant_id = $1`,
      [ctx.tenantId, ...parcela_ids]
    );
    ...
    for (const parcela of parcelasResult.rows) {
      ...
      // Verificar se ja existe pagamento - filter by tenant_id
      const pagamentoExistente = await dbQuery(
        `SELECT id FROM pagamentos_intermediacao WHERE parcela_id = $1 AND tenant_id = $2`,
        [parcela.id, ctx.tenantId]
      );
    ...
    for (const parcela of parcelasValidas) {
      // Criar registro de pagamento - include tenant_id
      const pagamentoResult = await dbQuery(
        `INSERT INTO pagamentos_intermediacao
         (tenant_id, parcela_id, beneficiario_id, valor, data_pagamento, metodo, referencia, registrado_por, created_at)
      ...
      // Atualizar status da parcela - filter by tenant_id
      await dbQuery(
      ...
      // Registrar auditoria - include tenant_id
      await dbQuery(
        `INSERT INTO log_auditoria_intermediacao
         (tenant_id, entidade, entidade_id, acao, usuario_id, usuario_nome, dados_novos, created_at)
    ...
    // Verificar vendas que podem ter sido totalmente pagas - filter by tenant_id
    ...
    for (const vendaId of vendasParaVerificar) {
      const parcelasPendentesResult = await dbQuery(
        `SELECT COUNT(*) as pendentes
         ...
         WHERE venda_id = $1 AND status != 'paga' AND status != 'cancelada' AND tenant_id = $2`,
        [vendaId, ctx.tenantId]
      );
