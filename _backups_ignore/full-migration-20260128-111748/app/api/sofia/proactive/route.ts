...
import { getTenant, listTenants, Tenant } from "@/lib/tenant-context";
import {
  sendTextMessage,
  formatPhoneNumber,
  isInstanceConnected,
...
async function runProactiveCheck(
  options: { dryRun?: boolean; tenantId?: number; userId?: string } = {}
): Promise<RunSummary> {
  const { dryRun = false, tenantId, userId } = options;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { dryRun = false, tenantId, userId } = body;
