...
export async function processarSelecaoImovel(
  ...
  const imoveisEncontrados = context.entities.imoveisEncontrados || [];
  const imovelSelecionado = imoveisEncontrados.find((i: any) => i.id === imovelId);
  ...
  if (context.current_flow === 'venda_imovel') {
    ...
    if (context.awaiting_response === 'confirmacao_agendamento') {
      const resultado = await confirmarAgendamento(
        nomeCliente,
        texto,
        context.entities?.imovelSelecionado || '',
        telefoneCliente,
        tenantId
      );
