...
async function handleGreeting(
  ...
  if (user.role === "corretor" && user.tenant_id) {
    try {
      const corretorId = await getCorretorIdByUserId(user.id);
      if (corretorId) {
        const leadsPayload = await getLeadsByCorretor(String(corretorId), user.tenant_id, {
          limit: 3,
          periodo: "semana",
        });
...
async function handleMetasFlow(
  ...
  if (intent.intent === 'metas_ranking') {
    try {
      const rankingData = await getRankingEquipe(user.tenant_id || 1, {
        limite: 10,
        periodo: 'mes',
  ...
  try {
    const metasData = await getMetasCorretor(String(corretorId), user.tenant_id || 1);
...
async function handleStatusFlow(
    ...
    if (context.current_flow === 'status' && context.awaiting_response === 'identificacao_cliente') {
      ...
      if (textTrimmed.length >= 2 && textTrimmed.length <= 100) {
        ...
        if (possibleCpf.length === 11) {
          // É um CPF
          const resultado = await getReservaStatus('', user.tenant_id || 1, possibleCpf);
          return buildStatusResponse(resultado, context);
        } else {
          // Assumir que é um nome
          const resultado = await getReservaStatus(textTrimmed, user.tenant_id || 1);
          return buildStatusResponse(resultado, context);
        }
      }
    }
  ...
  // Buscar no CV CRM
  const resultado = await getReservaStatus(nomeCliente || '', user.tenant_id || 1, cpf);
  return buildStatusResponse(resultado, context);
}
...
async function handleComissaoFlow(
  ...
  const { comissoes, resumo } = await getComissoesCorretor(String(corretorId), user.tenant_id || 1);
