...
export async function enviarFollowUpAutomatico(
  ...
  try {
    ...
    await pool.query(
      ...
      [leadId, tenantId, 'follow_up_enviado', tipoFollowUp]
    );
 ...
 */
export async function executarFollowUpsBatch(tenantId: number): Promise<{
  processados: number;
  enviados: number;
  erros: number;
}> {
  ...
  try {
    ...
    for (const { id: leadId } of leadsResult.rows) {
      ...
      try {
        const verificacao = await verificarFollowUpNecessario(leadId, tenantId);

        if (verificacao && verificacao.precisa) {
          const sucesso = await enviarFollowUpAutomatico(leadId, tenantId, verificacao.tipo);
          if (sucesso) {
            enviados++;
          } else {
            erros++;
