...
export class LeadsHistoricoAgent extends BaseSyncAgent<CVCRMHistoricoData, DBHistoricoData> {
  constructor() {
    const config: AgentConfig = {
      ...
      endpoints: [
        {
          path: '/api/v1/comercial/leads_historico_situacoes',
        ...
        {
          path: '/api/v1/comercial/leads_corretores',
          tokenEnvVar: 'CVCRM_TOKEN_LEAD',
          method: 'GET',
          supportsIncremental: true,
          paginationType: 'offset',
  ...
  async sync(fullSync = false) {
    console.log('[LeadsHistoricoAgent] Starting history sync...');

    const results = {
      historico: await this.syncTable(
    ...
    console.log('[LeadsHistoricoAgent] Sync completed:', {
      historico: results.historico.total,
